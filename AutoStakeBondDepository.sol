...
contract StagBondDepository is Ownable {
    ...
    address public immutable Stag; // token given as payment for bond
    address public immutable sStag; //staked bond token
    ...
    struct Bond {
        uint gonsPayout; // sStag gons remaining to be paid
        uint payout; // Stag amount when we bond
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 Stag ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) Stg
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( Stag ).safeTransfer( DAO, fee ); 
        }
        
        // total debt is increased
        totalDebt = totalDebt.add( value ); 
                
        IERC20(Stag).approve(staking, payout);
        IStaking(staking).stake(payout, address(this));
        IStaking(staking).claim(address(this));
        uint stakeGons = IsStag(sStag).gonsForBalance(payout);
    ...
    /** 
     *  @notice redeem bond for user.  We're keeping _stake for compat but we ignore it
    ...
    function redeem( address _recipient, bool _stake ) external returns ( uint ) {        
        ...
        require(percentVested >= 10000); //Fully vested only
        delete bondInfo[_recipient];    //delete user info
        uint _amount = IsStag(sStag).balanceForGons(info.gonsPayout);
        emit BondRedeemed(_recipient, _amount, 0);
        IERC20(sStag).safeTransfer(_recipient, _amount);
        return _amount;
        /*if ( percentVested >= 10000 ) { // if fully vested
            delete bondInfo[ _recipient ]; // delete user info
            emit BondRedeemed( _recipient, info.payout, 0 ); // emit bond data
            //return stakeOrSend( _recipient, _stake, info.payout ); // pay user everything due
            return stakeStag(_recipient, info.payout);
        }
        // Removing the ability to withdraw before full vested
    ...
    /**
     *  @notice allow user to stake payout automatically. replaced with stakeStag
     *  @param _stake bool
     *  @param _amount uint
     *  @return uint
     */
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( Stag ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( Stag ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( Stag ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( Stag ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( Stag ).totalSupply();
