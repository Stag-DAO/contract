...
//Keeping this name since it's become it's own standard at this point
interface IOHMERC20 {
...
contract StagTreasury is Ownable {
    ...
    /**
        @notice allow approved address to deposit an asset for Stag
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        IERC20Mintable( Stag ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Stag for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IOHMERC20( Stag ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sStag ).balanceOf( msg.sender ); // Can only borrow against sOHM held
    ...
    function repayDebtWithOHM( uint _amount ) external {
        ...
        IOHMERC20( Stag ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Stag, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Stag ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Stag ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Stag valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = _amount.mul( 10 ** IERC20( Stag ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
